...
- `startKit()` (chamado no BaseLayout) revela os `data-cube` por IntersectionObserver e liga o deck: `scroll-snap-type: y mandatory` na janela, cada `main > section` e o `footer` com `min-height: 100dvh`, `scroll-snap-align: start` e `scroll-snap-stop: always`. O JS NÃO intercepta roda, teclado nem toque: só lê `scrollY` e pinta rotação, blur, opacidade, tom e dobra. É exatamente o que o olek faz no desktop (o `.sections` dele tem o mesmo snap, com divs `.snap` vazias como alvos).
- Estudo de 12/09/2026, depois de quatro rodadas de heurística de roda que passavam nos testes sintéticos e travavam no trackpad real: interceptar `wheel` obriga a reimplementar a física do trackpad (inércia, pausas, inversões, dedo novo sem pausa), e cada regra quebra num caso real. O navegador já tem essa física. O que impedia o snap nativo era a geometria: o snap mira a caixa já transformada e, com a rotação no centro, o alvo fugia. Com a dobradiça na borda (topo de quem entra, base de quem sai) o alvo não se move. Seção mais alta que a tela não precisa de rolagem interna: área de snap maior que a janela rola livre por dentro (spec de Scroll Snap, 6.2) e só encaixa nas costuras; `scroll-snap-stop: always` dá um slide por gesto de graça. Regra que fica: NUNCA interceptar a entrada de scroll; se o snap nativo não serve, a solução é geometria, não JS.
...
- Dobra na costura (de jh3y/lid-plane): `transform-origin` 50% 0 em quem entra, 50% 100% em quem sai, projeção paralela sem `perspective`, blur crescendo com a distância da dobradiça via `--fold` (0 em repouso, 1 no meio da troca) nas camadas de `backdrop-filter` do topo e do rodapé.
- `content-visibility` fora: com altura natural ele encolhe a seção fora da tela e desloca as posições.
